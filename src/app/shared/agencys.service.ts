...
export class AgencyService {
  private base_url='http://localhost:3000/Agency';
...
all():Observable<Agency>{
  ...
  getByid(id:number):Observable<Agency>{
    return this.http.get<Agency>(this.base_url + '/' +id).pipe(retry(2),catchError(this.handleError));

  }

   // update team by Id the
   update(item :any){
     return this.http.put<Agency>(this.base_url + '/' +item.id,JSON.stringify(item),this.httpOptions).pipe(retry(2),catchError(this.handleError));
   }

    // delete cars
    delete(id:number){
      return this.http.delete<Agency>(this.base_url + '/' +id,this.httpOptions).pipe(retry(2),catchError(this.handleError));
...
populateForm(agency: any) {
  this.form.patchValue(_.omit(agency));
}
